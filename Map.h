...
private:
	...
	class iter :public Iterator<KEY, T>
	{
	private:
		queue <TreeNode*> tree;

		void queue_filling(TreeNode* start) //recursive filling of the queue
		{
			if (!start)
			{
				return;
			}
			queue_filling(start->left);
			this->tree.enqueue(start);
			queue_filling(start->right);
		}
	public:
		iter(TreeNode* Node)
		{
		...
		{
			if (has_next()) return tree.peek()->key;
			//else throw std::out_of_range("The next element doesn't exist!");
		}
		 T next_data() override
		{
			if (has_next()) return tree.dequeue()->data;
			else throw std::out_of_range("The next element doesn't exist!");
		}
	...
	TreeNode* contains(KEY key, TreeNode* Root)
	{
		if (Root)
		{
			if (key < Root->key)
